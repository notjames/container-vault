...
podTemplate(label: "${project_name}", containers: [
    ...
    secretVolume(mountPath: '/home/jenkins/.docker/', secretName: 'samsung_cnct+containervaultrw')
  ]) {
    node("${project_name}") {
      customContainer('docker') {
        ...
        stage('Test') {
          kubesh "docker run -e ENVIRONMENT=test --rm -t ${project_name}:${env.JOB_BASE_NAME}.${env.BUILD_ID}"
        }
